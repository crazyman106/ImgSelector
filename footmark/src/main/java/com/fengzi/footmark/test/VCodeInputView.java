...
public class VCodeInputView extends android.support.v7.widget.AppCompatEditText {
     ...
     * 底部线的颜色   默认GRAY
     */
    private int bottomLineColor = Color.parseColor("#66999999");
    ...
    public VCodeInputView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
    ...
    private void getAtt(AttributeSet attrs) {
        TypedArray typedArray = mContext.obtainStyledAttributes(attrs, R.styleable.VCodeInputView);
        maxCount = typedArray.getInt(R.styleable.VCodeInputView_maxCount, maxCount);
        circleColor = typedArray.getColor(R.styleable.VCodeInputView_circleColor, circleColor);
        bottomLineColor = typedArray.getColor(R.styleable.VCodeInputView_bottomLineColor, bottomLineColor);
        radius = typedArray.getDimensionPixelOffset(R.styleable.VCodeInputView_radius, radius);

        divideLineWidth = typedArray.getDimensionPixelSize(R.styleable.VCodeInputView_divideLineWidth, divideLineWidth);
        divideLineColor = typedArray.getColor(R.styleable.VCodeInputView_divideLineColor, divideLineColor);
        psdType = typedArray.getInt(R.styleable.VCodeInputView_psdType, psdType);
        rectAngle = typedArray.getDimensionPixelOffset(R.styleable.VCodeInputView_rectAngle, rectAngle);
        focusedColor = typedArray.getColor(R.styleable.VCodeInputView_focusedColor, focusedColor);
...
//        drawPsdCircle(canvas);
    ...
    private void drawText(Canvas canvas) {
        ...
        int everySpan = width / 6;
        Rect rect = new Rect();
        paint.getTextBounds("5", 0, 1, rect);

        for (int index = 0; index < length; index++) {
            canvas.drawText(text.substring(index, index + 1), everySpan / 2 + index * everySpan - rect.right / 2, height / 2 - rect.top * 3 / 4, getPaint());
        }
    }
    ...
    private void drawBottomLine(Canvas canvas) {
        int space = width / 6;
        int padding = space / 6;
        int lineWidth = space - 2 * padding;
        for (int i = 0; i < maxCount; i++) {
            canvas.drawLine(padding + i * space,
                    height,
                    padding + lineWidth + i * space,
                    height,
                    bottomLinePaint);
        }
    ...
    private void drawItemFocused(Canvas canvas, int position) {
        ...
        focusedRecF.set(position * divideLineWStartX, 0, (position + 1) * divideLineWStartX,
                height);
        canvas.drawRoundRect(focusedRecF, rectAngle, rectAngle, getPaint(3, Paint.Style.STROKE, redColor));
    }

    int redColor = Color.parseColor("#77f54548");
    int whiteColor = Color.parseColor("#ffffff");

    private void drawCursor(final Canvas canvas, final int position) {
        if (position > maxCount - 1) {
            return;
        }
        canvas.drawLine(position * divideLineWStartX + divideLineWStartX / 2, height * 2 / 5, position * divideLineWStartX + divideLineWStartX / 2, height * 5 / 6, getPaint(3, Paint.Style.STROKE, redColor ));
    }
